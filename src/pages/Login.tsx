...
export default function Login() {
  ...
  const handleGoogle = async () => {
    try { await signInWithGoogle(); } catch { toast.error(t("auth.googleFailed")); }
  };
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-center text-sm text-muted-foreground">
            {t("auth.needAccount")} <Link to="/signup" className="text-primary hover:underline">{t("auth.signUpLink")}</Link>
          </p>
        </CardContent>
      </Card>
    </div>
